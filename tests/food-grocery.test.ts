...
describe("смешанные единицы", () => {
  ...
  /** Жидкость покупают объёмом, поэтому вес переводится в миллилитры по плотности,
   *  а не приравнивается к ним: у молока это 1.03 г/мл. */
  it("миллилитры и граммы одного продукта сливаются в одну строку — в миллилитрах", () => {
    ...
    expect(milk[0]!.unit).toBe("мл");
    expect(milk[0]!.qty).toBeCloseTo(150 / 1.03 + 200, 0);
  });

  it("жидкость показывается объёмом, даже если в рецептах записана весом", () => {
    const g = buildGroceryList([{ meals: [meal("масло оливковое", 46, "г")] }]);
    const oil = g.items.find(i => i.name === "масло оливковое")!;
    expect(oil.unit).toBe("мл");
    expect(oil.qty).toBeCloseTo(46 / 0.92, 0);   // масло легче воды
  });
