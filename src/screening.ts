 ...
 *
 * Подключён к форме еды (шаг 8) решением Сердара 2026-08-08 — до этого модуль был написан,
 * обоснован и мёртв: вопросов никто не задавал. Вопросов тринадцать, но подряд их никто
 * не видит: сначала два «ворот» («громко храплю…», «просыпаюсь ночью поесть…»), и остальное
 * раскрывается только при «да». Вердикты показываются на «Итогах» и только при флаге,
 * а ночное питание вдобавок смягчает дефицит через `applySafety`.
 */
