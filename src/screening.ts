 ...
 *
 * ponytail: модуль НЕ ПОДКЛЮЧЁН к интерфейсу — вопросов STOP-Bang и NES человеку никто
 * не задаёт, вызовов из экранов нет (проверено аудитом 2026-08-08). Оставлен намеренно,
 * а не по недосмотру: наука под ним собрана (`X19`, `X21`, `X23`), а `applySafety` уже
 * умеет смягчать дефицит по его результату. Развилка простая и требует решения Сердара:
 * либо добавить полдюжины вопросов в форму еды и подать сюда ответы, либо удалить файл
 * вместе с тестами. Пока не решено — это балласт, и честнее так и называть его балластом.
 */
