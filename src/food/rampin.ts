...
/**
 * Нужна ли сейчас привычная еда — паста, жаркое, запеканки, а не миска овощей.
 *
 * Первая версия делала обратное: ставила ПОТОЛОК по плотности энергии (2.6 ккал/г в начале,
 * 2.05 к середине) и выбрасывала самые плотные блюда — ровно ту привычную еду, ради которой
 * всё и затевалось. Заодно рычаг был мёртвым: на реальном наборе потолок 2.6 убирал 7 рецептов
 * из 190, а 2.05 — четырнадцать. То есть он либо не делал ничего, либо делал вред.
 *
 * Правильная сторона — ПОЛ, а не потолок: пока человек ест почти как раньше, блюда должны быть
 * плотными, иначе тот же калораж превращается в гору еды. Порог не абсолютный, а относительный
 * (верхняя половина набора по плотности) — так же, как устроены упрощение дня и бюджет:
 * на любом наборе рецептов работает одинаково и не ломается, когда рецепты добавляют.
 *
 * Действует в первой половине вхождения; дальше набор возвращается к обычному.
 */
export function prefersFamiliar(ramp: RampState): boolean {
  return ramp.active && ramp.total > 0 && ramp.day <= ramp.total / 2;
}
